...
<!--            <router-link to="/show-detail" id="cttg_btn" class="categories__btn d-flex" style="text-decoration: none;">{{$t('View all')}}<span class="categories__span">></span></router-link>-->
      ...
      <section>
        <div class="container">
          ...
          <div class="row d-flex justify-content-center" v-if="getProducts">
              <router-link v-for="(product, index) in getProducts" :key="index" :to="'/show-detail/'+product.id" class="col-xl-2 col-lg-3 col-md-3 col-sm-4" id="card_like">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      ...mapActions(['GETPRODUCTSFUNC', 'GETPRODUCTSBYCATEGORIES', 'GETPRODUCTSBYSUBCATEGORIES', 'GETPRODUCTSBYSUBSUBCATEGORIES']),
      showBasket(i){
