...
<!--          <router-link to="/show-detail" id="cttg_btn" class="categories__btn d-flex" style="text-decoration: none;">{{$t('View all')}}<span class="categories__span">></span></router-link>-->
    ...
    <section>
      <div class="container" v-if="getProductsByCategories[router_id]">
        <h3 class="recomidation_text-big mt-5">{{$t('Products')}}</h3>
        <div class="row d-flex justify-content-center" v-if="getProductsByCategories[router_id].products">
          <router-link v-for="(product, index) in getProductsByCategories[router_id].products" :key="index" :to="'/show-detail/'+product.id" class="col-xl-2 col-lg-3 col-md-3 col-sm-4" id="card_like">
...
export default {
  ...
  methods:{
    ...
    ...mapActions(['GETPRODUCTSFUNC', 'GETPRODUCTSBYCATEGORIES']),
    showBasket(i){
